...
public class HotelBookingServlet extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
	    ...
	    try {
			JSONObject hotelsData = getHotels(destination, checkInDate, checkOutDate, numOfPeople, hotelBrand, budget, amenities);
			request.setAttribute("hotelsData", hotelsData);			
			saveHotelsIntoDB(hotelsData, checkInDate, checkOutDate, destination, budget, numOfPeople, roomType, specialRequests);
			RequestDispatcher dispatcher = request.getRequestDispatcher("/hotelDetails.jsp");
	...
	private JSONObject getHotels(String destination, LocalDate checkInDate, LocalDate checkOutDate, int numOfPeople, String hotelBrand, int budget, String[] amenities) throws IOException, InterruptedException {
	...
	private void saveHotelsIntoDB(JSONObject hotelsData, LocalDate checkInDate, LocalDate checkOutDate,
        ...
        for (int i = 0; i < hotelList.size(); i++) {
    		...
    		Hotel hotelObj = new Hotel(hotelID, hotelName, checkInDate, checkOutDate, lowestPrice, destination, budget, numOfPeople, roomType, specialRequests, checkInTime, checkOutTime, overallRating, hotelClass, hotelLink, image);
